...
export const FolderDropdown: React.FC<{
  ...
  const renderFolderTree = (items: Folder[], depth = 0) => {
    ...
    return tree.map((folder) => {
      ...
      return (
        <div key={folder.id}>
          <div className="w-full px-4 py-2 flex items-center justify-between hover:bg-wash transition-colors duration-150 group">
            <div
              className="flex items-center space-x-3 flex-1 min-w-0"
              style={{ marginLeft: `${depth * 20}px` }}
            >
              <FiFolder
                className={`w-4 h-4 shrink-0 transition-colors ${isSaved ? "text-primary" : "text-faint group-hover:text-primary"}`}
              />
  ...
  const listBody = isLoading ? (
    ...
    <div className="px-4 py-8 text-center">
      ...
      <p className="text-sm text-muted font-medium">Failed to load folders</p>
    </div>
  ) : (
    <div className="py-2">
      {folders.length === 0 ? (
      ...
      {isCreatingNew ? (
        <div className="flex items-center space-x-2">
          <input
            ...
            className="flex-1 px-3 py-1.5 border border-gray-300 dark:border-neutral-700 bg-background rounded-md text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-foreground"
            autoFocus
          />
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`hidden lg:block absolute left-0 bg-background rounded-md border border-line shadow-xl overflow-hidden z-70 ${className}`}
          >
