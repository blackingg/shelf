...
export const FolderSelectDropdown: React.FC<FolderSelectDropdownProps> = ({
      ...
      return (
        <div key={folder.id}>
          <button
            ...
            className={`w-full px-4 py-3 flex items-center justify-between hover:bg-wash transition-colors duration-150 group ${
          ...
          {isCreatingSubfolderHere && (
            <div
              className="flex items-center gap-2 px-4 py-2 bg-gray-50/80 dark:bg-neutral-800/60 border-y border-line-subtle"
            ...
            >
              ...
              <input
                ...
                className="flex-1 min-w-0 px-2 py-1 border border-gray-300 dark:border-neutral-700 bg-background rounded-sm text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-foreground"
              />
              <button
                type="button"
                onClick={(e) => {
  ...
  // Click outside handler (desktop dropdown only — the mobile sheet has its own backdrop)
  useEffect(() => {
    ...
    <div className="space-y-1 py-1">
      {folders.length === 0 ? (
        <div className="px-4 py-8 text-center">
          ...
          <p className="text-sm text-muted font-medium">No folders yet</p>
        </div>
      ) : (
  ...
  const createFolderFooter = (
    <div className="p-3 bg-gray-50/30 dark:bg-neutral-800/30">
      {isCreatingNew ? (
        <div className="flex items-center space-x-2">
          <input
            ...
            autoFocus
            className="flex-1 px-3 py-1.5 border border-gray-300 dark:border-neutral-700 bg-background rounded-md text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 text-foreground"
          />
      ...
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-neutral-800/50 border border-transparent rounded-sm transition-all duration-200 group ${
          isOpen
            ...
            : "hover:bg-gray-100 dark:hover:bg-neutral-800 border-line"
        }`}
      >
        <div className="flex items-center gap-3">
          ...
          />
          <span className={selectedFolder ? "text-foreground" : "text-gray-400"}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="hidden lg:block absolute left-0 right-0 top-full mt-2 bg-background border border-line rounded-md shadow-xl overflow-hidden z-20"
          >
            <div className="flex items-center justify-between p-3 border-b border-line bg-gray-50/50 dark:bg-neutral-800/50">
            ...
            <div className="border-t border-line">{createFolderFooter}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
