...
type Resource =
  ...
  | {
      type: "book";
      data: Partial<Book> & { id: string; title: string; author: string };
    };
...
interface UnifiedTableProps {
  ...
  onSelectionChange?: (type: "book" | "folder", ids: string[]) => void;
}
...
export const UnifiedTable = ({
  ...
  const isSelected = (resource: Resource) => {
    if (resource.type === "book")
      return selectedBookIds.includes(resource.data.id);
    return selectedFolderIds.includes(resource.data.id);
  };
  ...
  const toggleSelection = (resource: Resource) => {
    ...
    if (resource.type === "book") {
      const newIds = selectedBookIds.includes(id)
        ? selectedBookIds.filter((i) => i !== id)
        : [...selectedBookIds, id];
      onSelectionChange("book", newIds);
    } else {
      const newIds = selectedFolderIds.includes(id)
        ? selectedFolderIds.filter((i) => i !== id)
        : [...selectedFolderIds, id];
      onSelectionChange("folder", newIds);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-md border border-gray-100 dark:border-white/5">
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center space-x-4">
                      <div className="w-8 h-10 bg-gray-50 dark:bg-neutral-800 rounded-sm flex-shrink-0 overflow-hidden border border-gray-100 dark:border-white/5 flex items-center justify-center">
                        {res.type === "book" ? (
                          res.data.coverImage ? (
                            <img
                              src={res.data.coverImage}
      ...
      {activeMenuId && menuPosition && (
        ...
        >
          {(() => {
            const res = resources.find((r) => r.data.id === activeMenuId);
            if (!res) return null;
