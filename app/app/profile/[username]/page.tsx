...
export default function ProfilePage() {
  ...
  const isOwner = currentUser?.username === decodedUsername;
  const { addNotification } = useNotifications();

  const { actions: userActions } = useUser();
  const { actions: folderActions } = useFolderActions();

  const { user, isLoading: isLoadingUser } = useUserByUsername(decodedUsername);
  ...
  } = useUserBooks({ username: decodedUsername, page, limit: pageSize });
  ...
  } = useUserFolders({ username: decodedUsername, page: folderPage, limit: pageSize });
        ...
        {isLoadingUser ? (
          ...
          <div className="max-w-7xl mx-auto px-6 py-24 text-center">
            ...
            <p className="text-gray-600 dark:text-neutral-400 mb-8 max-w-sm mx-auto">
              The user @{decodedUsername} doesn&apos;t exist.
            </p>
            <button
